#
# BioPerl module for Bundle::Bio::Root::Storable
#
...
Bundle::Bio::Root::Storable - object serialisation methods

=head1 SYNOPSIS

  my $storable = Bundle::Bio::Root::Storable->new();

  # Store/retrieve using class retriever
  my $token     = $storable->store();
  my $storable2 = Bundle::Bio::Root::Storable->retrieve( $token );
...
$Bundle::Bio::Root::Storable::BINARY to false.
...
# Let the code begin...
package Bundle::Bio::Root::Storable;
...
=head2 new
  ...
  Function  : Builds a new Bundle::Bio::Root::Storable inhereting object
  Returntype: Bundle::Bio::Root::Storable inhereting object
  Exceptions: 
  Caller    : 
  Example   : $storable = Bundle::Bio::Root::Storable->new()
...
=head2 statefile
  ...
  Returntype: string
  Exceptions: 
  Caller    : Bundle::Bio::Root::Storable->store
...
sub statefile{
  ...
  if( ! $self->{$key} ){ # Create a new statefile
    ...
    my( $fh, $file ) = Bundle::Bio::Root::IO->new->tempfile( @args );
...
#  $self->{$key} ||= $Bundle::Bio::Root::IO::TEMPDIR;
...
sub store{
  ...
  my $io = Bundle::Bio::Root::IO->new( ">$statefile" );
  $io->_print( $store_obj );
...
sub serialise{
  ...
  foreach my $key( keys( %$self ) ){
    ...
    # Bundle::Bio::Root::Storable obj: save placeholder
    elsif( ref($value) =~ /^Bundle::Bio::/ and $value->isa('Bundle::Bio::Root::Storable') ){
      # Bundle::Bio::Root::Storable
      $store_obj->{$key} = $value->new_retrievable( %retargs );
      next;
    }

    # Arrayref value. Look for Bundle::Bio::Root::Storable objs
    elsif( ref( $value ) eq 'ARRAY' ){
      my @ary;
      foreach my $val( @$value ){
	if( ref($val) =~ /^Bundle::Bio::/ and $val->isa('Bundle::Bio::Root::Storable') ){
    ...
    # Hashref value. Look for Bundle::Bio::Root::Storable objs
...
=head2 retrieve
  ...
  Exceptions: 
  Caller    : 
  Example   : my $obj = Bundle::Bio::Root::Storable->retrieve( $token );
...
=head2 _freeze
  ...
  Function  : Converts whatever is in the the arg into a string.
              ...
              depending on the value of $Bundle::Bio::Root::BINARY
  Returntype: 
  Exceptions: 
  Caller    : 
  Example   : 
...
=head2 _thaw
  ...
  Function  : Converts the string into a perl 'whatever'.
              ...
              value of $Bundle::Bio::Root::BINARY.
