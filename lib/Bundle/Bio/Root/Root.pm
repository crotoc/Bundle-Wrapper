...
Bundle::Bio::Root::Root - Hash-based implementation of Bundle::Bio::Root::RootI
...
=head1 SYNOPSIS
  ...
  $obj->throw( -class => 'Bundle::Bio::Root::BadParameter',
               -text  => "Can not open file $file",
               -value  => $file );

  # Want to see debug() outputs for this object

  my $obj = Bundle::Bio::Object->new(-verbose=>1);

  my $obj = Bundle::Bio::Object->new(%args);
  $obj->verbose(2);
...
This is a hashref-based implementation of the Bundle::Bio::Root::RootI
interface.  Most Bioperl objects should inherit from this.

See the documentation for L<Bundle::Bio::Root::RootI> for most of the methods
implemented by this module.  Only overridden methods are described
here.

=head2 Throwing Exceptions

One of the functionalities that L<Bundle::Bio::Root::RootI> provides is the
ability to L<throw>() exceptions with pretty stack traces. Bundle::Bio::Root::Root
...
L<Bundle::Bio::Root::RootI>-based exception throwing facilty.
...
This should be one of the classes defined in L<Bundle::Bio::Root::Exception>,
or a custom error of yours that extends one of the exceptions
defined in L<Bundle::Bio::Root::Exception>.
...
Note that Bundle::Bio::Root::Exception does not need to be imported into
your module (or script) namespace in order to throw exceptions
via Bundle::Bio::Root::Root::throw(), since Bundle::Bio::Root::Root imports it.
...
Here's an example. See the L<Bundle::Bio::Root::Exception> module for
other pre-defined exception types:

   try {
    open( IN, $file) || $obj->throw( -class => 'Bundle::Bio::Root::FileOpenException',
                                     -text => "Cannot open file $file for reading",
                                     -value => $!);
   }
   catch Bundle::Bio::Root::BadParameter with {
...
use strict;
use Bundle::Bio::Root::IO;

use base qw(Bundle::Bio::Root::RootI);

our ($DEBUG, $ID, $VERBOSITY, $ERRORLOADED, $CLONE_CLASS);


BEGIN {
    $ID        = 'Bundle::Bio::Root::Root';
    $DEBUG     = 0;
    $VERBOSITY = 0;
    $ERRORLOADED = 0;
    ...
    if( not $main::DONT_USE_ERROR ) {
        if ( eval "require Error"  ) {
            import Error qw(:try);
            require Bundle::Bio::Root::Exception;
            $ERRORLOADED = 1;
    ...
    for my $class (qw(Clone::Fast Clone Storable)) {
        ...
        if (!$@) {
            ...
            if ($class eq 'Clone::Fast') {
                *Bundle::Bio::Root::Root::_dclone = sub {shift; return Clone::Fast::clone(shift)};
            } elsif ($class eq 'Clone') {
                *Bundle::Bio::Root::Root::_dclone = sub {shift; return Clone::clone(shift)};
            } else {
                *Bundle::Bio::Root::Root::_dclone = sub {shift; return Storable::dclone(shift)};
            }
            last;
        }
    }
    if (!defined $CLONE_CLASS) {
        *Bundle::Bio::Root::Root::_dclone = sub {
...
sub clone {
    ...
    __PACKAGE__->throw("Can't call clone() as a class method") unless
        ref $orig && $orig->isa('Bundle::Bio::Root::Root');
...
=head2 throw
 ...
 Usage   : $obj->throw("throwing exception message");
           or
           $obj->throw( -class => 'Bundle::Bio::Root::Exception',
 ...
 Function: Throws an exception, which, if not caught with an eval or
           ...
           (i.e., a simple string is given), A Bundle::Bio::Root::Exception
           is thrown.
 Returns : n/a
 Args    : A string giving a descriptive error message, optional
           ...
           '-class'  a string for the name of a class that derives
                     ...
                     defined in Bundle::Bio::Root::Exception.
                     Default class: Bundle::Bio::Root::Exception
...
sub throw {
    ...
    if ($ERRORLOADED) {
        ...
        # If the error is not derived from Bundle::Bio::Root::Exception,
        # we can't guarantee that the Error's value was set properly
        # and, ipso facto, that it will be catchable from an eval{}.
        # But chances are, if you're re-throwing non-Bundle::Bio::Root::Exceptions,
        ...
        if (ref($args[0])) {
            if( $args[0]->isa('Error')) {
                ...
                my $class = "Bundle::Bio::Root::Exception";
...
=head2 _load_module
 ...
 Title   : _load_module
 Usage   : $self->_load_module("Bundle::Bio::SeqIO::genbank");
