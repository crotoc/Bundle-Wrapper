#
# BioPerl module for Bundle::Bio::Root::Test
#
...
Bundle::Bio::Root::Test - A common base for all Bioperl test scripts.

=head1 SYNOPSIS

  use lib '.'; # (for core package tests only)
  use Bundle::Bio::Root::Test;
  ...
  # Bundle::Bio::Root::Test rewraps Test::Most, so one can carry out tests with
...

{
    ...
    sub Test::Warn::_canonical_got_warning {
        ...
        my $warn_kind = $called_from eq 'Carp' ? 'carped' : ($called_from =~ /Bundle::Bio::/ ? 'Bioperl' : 'warn');
...
our @EXPORT = (@Test::Most::EXPORT,
               #@Bundle::Bio::Root::Test::Warn::EXPORT,
...
=head2 test_skip
 ...
 Args    : -tests               => int (REQUIRED, the number of tests that are
           ...
           -requires_module     => str (as above, but for just one module)
           -requires_executable => Bundle::Bio::Tools::Run::WrapperBase instance
...
sub _skip {
    ...
    if ($req_exe && (!ref($req_exe) || !$req_exe->isa('Bundle::Bio::Tools::Run::WrapperBase'))) {
        die "-requires_exe takes an argument of type Bundle::Bio::Tools::Run::WrapperBase";
    }
