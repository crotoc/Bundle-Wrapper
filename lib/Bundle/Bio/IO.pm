#
# BioPerl module for Bundle::Bio::Root::IO
#
...
Bundle::Bio::Root::IO - module providing several methods often needed when dealing with file IO

=head1 SYNOPSIS

    # utilize stream I/O in your module
    $self->{'io'} = Bundle::Bio::Root::IO->new(-file => "myfile");
    ...
    $path = Bundle::Bio::Root::IO->catfile($dir, $subdir, $filename);
...
sub _initialize_io {
    ...
    if($url){
        ...
        if($HAS_LWP) { #use LWP::UserAgent
            ...
            $input = $tempfile;
            $file  = $tempfile;
        } else { #use Bundle::Bio::Root::HTTPget
            #$self->warn("no lwp");

            $fh = Bundle::Bio::Root::HTTPget::getFH($url);
        }
        }
...
=head2 format
 ...
 Function: Get the format of a Bundle::Bio::Root::IO sequence file or filehandle. Every
           object inheriting Bundle::Bio::Root::IO is guaranteed to have a format.
...
=head2 variant
 ...
 Title   : format
 Usage   : $self->format($newval)
 Function: Get the variant of a Bundle::Bio::Root::IO sequence file or filehandle.
 ...
 Args    : none
 Note    : The Bundle::Bio::Root::IO-implementing modules that require access to variants
...
sub variant {
    ...
    if (defined $variant) {
        ...
        my %ok_variants = eval $var_name; # e.g. %Bundle::Bio::Assembly::IO::ace::variant
...
=head2 exists_exe
 ...
 Usage   : $exists = $obj->exists_exe('clustalw');
           $exists = Bundle::Bio::Root::IO->exists_exe('clustalw')
           $exists = Bundle::Bio::Root::IO::exists_exe('clustalw')
...
sub exists_exe {
    ...
    if($FILESPECLOADED) {
        foreach my $dir (File::Spec->path()) {
            my $f = Bundle::Bio::Root::IO->catfile($dir, $exe);
            return $f if( -f $f && -x $f );
        }
    }
    return 0;
...
=head2 tempfile
 ...
 Args    : named parameters compatible with File::Temp: DIR (defaults to
           $Bundle::Bio::Root::IO::TEMPDIR), TEMPLATE, SUFFIX.
...
=head2  tempdir
 ...
 Function: Creates and returns the name of a new temporary directory.
           ...
           temp directory. Use $Bundle::Bio::Root::IO::TEMPDIR for that. Calling this
...
=head2 catfile
 ...
 Title   : catfile
 Usage   : $path = Bundle::Bio::Root::IO->catfile(@dirs,$filename);
...
=head2 rmtree
 ...
 Title   : rmtree
 Usage   : Bundle::Bio::Root::IO->rmtree($dirname );
