package	Bundle::Bio::Root::Utilities;
use strict;


=head1 NAME

Bundle::Bio::Root::Utilities - General-purpose utility module
...
=head2 Object Creation
    ...
    use Bundle::Bio::Root::Utilities qw(:obj);
    $Util->some_method();

    # Create an object manually:
    use Bundle::Bio::Root::Utilities;
    my $util = Bundle::Bio::Root::Utilities->new();
    $util->some_method();
...
namespace representing a Bundle::Bio::Root::Utilities object. This saves you
from creating your own Bundle::Bio::Root::Utilities object via
C<Bundle::Bio::Root::Utilities-E<gt>new()> or from prefixing all method calls with
C<Bundle::Bio::Root::Utilities>, though feel free to do these things if desired.
Since there should normally not be a need for a script to have more
than one Bundle::Bio::Root::Utilities object, this module thus comes with it's
own singleton.
...
Inherits from L<Bundle::Bio::Root::Root>, and uses L<Bundle::Bio::Root::IO> 
and L<Bundle::Bio::Root::Exception>.
...
use base qw(Bundle::Bio::Root::Root Exporter);
@EXPORT_OK   = qw($Util);
...
#------------
    ...
    if(system(@cmd) != 0) {
        $self->throw( -class => 'Bundle::Bio::Root::SystemException',
...
#---------------------
	...
	$client->{'_input_type'} = "Glob";
        ...
        $self->throw(-class=>'Bundle::Bio::Root::IOException',
	...
	$file = "$gzip -cd $file |"
      ...
      require FileHandle;
      $FH = FileHandle->new();
      open ($FH, $file) || $self->throw(-class=>'Bundle::Bio::Root::FileOpenException',
...
=head2 get_newline
 ...
 Purpose   : Determine the character(s) used for newlines in a given file or
           : input stream. Delegates to Bundle::Bio::Root::Utilities::get_newline()
 Example   : $data = $object->get_newline(-CLIENT => $anObj,
           :                                   -FILE =>'usr/people/me/data.txt')
 Argument  : Same arguemnts as for create_filehandle().
 Returns   : Reference to a FileHandle object.
 Throws    : Propogates any exceptions thrown by Bundle::Bio::Root::Utilities::get_newline().
...
#---------------
  ...
  if($@ =~ /Timed out!/) {
    $self->throw("Timed out while waiting for input.",
		 "Timeout period = $TIMEOUT_SECS seconds.\nFor longer time before timing out, edit \$TIMEOUT_SECS in Bundle::Bio::Root::Utilities.pm.");
...
=head2 file_flavor
 ...
 Throws    : Exception if argument is not a file
           : Propogates any exceptions thrown by Bundle::Bio::Root::Utilities::get_newline().
...
=head2 mail_authority
 ...
 Purpose  : Syntactic sugar to send email to $Bundle::Bio::Root::Global::AUTHORITY
